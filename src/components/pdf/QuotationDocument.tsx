...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    borderBottomWidth: 2, borderBottomStyle: 'solid', borderBottomColor: '#1e3a5f',
  },
  ...
  statusBadge: {
    backgroundColor: '#e8f0fe', color: '#1e3a5f',
    paddingVertical: 4, paddingHorizontal: 8, borderRadius: 4,
    fontSize: 8, fontFamily: 'Helvetica-Bold', textAlign: 'right', marginTop: 4,
  },
  ...
  tableRow: {
    ...
    borderBottomWidth: 1, borderBottomStyle: 'solid', borderBottomColor: '#e5e7eb',
  },
  ...
  totalsBox: {
    width: 220, paddingTop: 10,
    borderTopWidth: 2, borderTopStyle: 'solid', borderTopColor: '#1e3a5f',
  },
  ...
  grandTotalRow: {
    ...
    borderTopWidth: 1, borderTopStyle: 'solid', borderTopColor: '#ddd',
  },
  ...
  notesBox: {
    marginTop: 20, padding: 10, backgroundColor: '#fff8ed', borderRadius: 2,
    borderLeftWidth: 3, borderLeftStyle: 'solid', borderLeftColor: '#f97316',
  },
  notesTitle: { fontSize: 8, fontFamily: 'Helvetica-Bold', color: '#f97316', marginBottom: 4 },
...
export function QuotationDocument({ quotation }: { quotation: any }) {
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.twoCol}>
          <View style={[styles.col, { marginRight: 20 }]}>
            ...
            {customer.email ? <Text style={styles.colValue}>{customer.email}</Text> : null}
            {customer.address ? <Text style={styles.colValue}>{customer.address}</Text> : null}
          </View>
          <View style={styles.col}>
            ...
            {quotation.shiftingDate ? <Text style={styles.colValue}>Date: {formatDate(quotation.shiftingDate)}</Text> : null}
            {quotation.validUntil ? <Text style={styles.colValue}>Valid Until: {formatDate(quotation.validUntil)}</Text> : null}
          </View>
        </View>
          ...
          <View style={styles.totalsBox}>
            {Number(quotation.subtotal) > 0 ? (
              <View style={styles.totalRow}>
                ...
                <Text style={styles.totalValue}>Rs.{Number(quotation.subtotal).toFixed(2)}</Text>
              </View>
            ) : null}
            {Number(quotation.laborCharges) > 0 ? (
              <View style={styles.totalRow}>
                <Text style={styles.totalLabel}>Labor Charges</Text>
                <Text style={styles.totalValue}>Rs.{Number(quotation.laborCharges).toFixed(2)}</Text>
              </View>
            ) : null}
            {Number(quotation.truckCharges) > 0 ? (
              <View style={styles.totalRow}>
                <Text style={styles.totalLabel}>Truck Charges</Text>
                <Text style={styles.totalValue}>Rs.{Number(quotation.truckCharges).toFixed(2)}</Text>
              </View>
            ) : null}
            {Number(quotation.insuranceAmount) > 0 ? (
              <View style={styles.totalRow}>
                <Text style={styles.totalLabel}>Insurance</Text>
                <Text style={styles.totalValue}>Rs.{Number(quotation.insuranceAmount).toFixed(2)}</Text>
              </View>
            ) : null}
            {Number(quotation.packingCharges) > 0 ? (
              <View style={styles.totalRow}>
                <Text style={styles.totalLabel}>Packing Charges</Text>
                <Text style={styles.totalValue}>Rs.{Number(quotation.packingCharges).toFixed(2)}</Text>
              </View>
            ) : null}
            ...
            <View style={styles.totalRow}>
              <Text style={styles.totalLabel}>GST ({Number(quotation.gstRate || 18)}%)</Text>
        ...
        {(quotation.notes || quotation.terms) ? (
          <View style={styles.notesBox}>
            ...
            {quotation.terms ? (
              <View style={quotation.notes ? { marginTop: 6 } : {}}>
