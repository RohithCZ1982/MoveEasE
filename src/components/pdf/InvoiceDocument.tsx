...
const styles = StyleSheet.create({
  ...
  paymentBox: {
    marginTop: 20, padding: 12, backgroundColor: '#f0fdf4', borderRadius: 2,
    borderLeftWidth: 3, borderLeftStyle: 'solid', borderLeftColor: '#22c55e',
  },
  paymentTitle: { fontSize: 9, fontFamily: 'Helvetica-Bold', color: '#15803d', marginBottom: 4 },
  paymentText: { fontSize: 9, color: '#555' },

  notesBox: {
    marginTop: 10, padding: 12, backgroundColor: '#fff8ed', borderRadius: 2,
    borderLeftWidth: 3, borderLeftStyle: 'solid', borderLeftColor: '#f97316',
  },
  notesTitle: { fontSize: 9, fontFamily: 'Helvetica-Bold', color: '#f97316', marginBottom: 4 },
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.twoCol}>
          <View style={[styles.col, { marginRight: 20 }]}>
            ...
            {customer.gstNumber ? <Text style={styles.colValue}>GST: {customer.gstNumber}</Text> : null}
          </View>
          <View style={styles.col}>
            ...
            {quotation.fromAddress ? <Text style={styles.colValue}>From: {quotation.fromAddress}</Text> : null}
            {quotation.toAddress ? <Text style={styles.colValue}>To: {quotation.toAddress}</Text> : null}
            {invoice.dueDate ? <Text style={styles.colValue}>Due: {formatDate(invoice.dueDate)}</Text> : null}
          </View>
        </View>
        ...
        {invoice.paymentMode ? (
          <View style={styles.paymentBox}>
            ...
            <Text style={styles.paymentText}>
              Mode: {invoice.paymentMode}{invoice.paymentDate ? `  |  Date: ${formatDate(invoice.paymentDate)}` : ''}
            </Text>
          </View>
        ) : null}
